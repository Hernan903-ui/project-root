...
const handleError = (error) => {
  ...
  return error.response?.data?.detail || error.response?.data?.message || error.message || "Error desconocido"
}
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSuppliers.rejected, (state, action) => {
        ...
        state.connectionIssue = action.payload?.includes("conexión") || action.payload?.includes("redirección") || false
        ...
        if (state.suppliers.length === 0 && !state.hasAttemptedFallback && process.env.NODE_ENV === "development") {
...
// Exportar thunks (añadir estas líneas);
